...
- **A reference cycle is rejected, by name.** UDPs may reference each other freely, but a chain that loops back on itself (`a` referencing `b` referencing `a`) can't be inlined. Each `PUT` on its own is valid — the loop only closes once the last one lands — so the cycle is caught when the graph is *used*: the request fails immediately with `User-defined processes reference each other in a cycle: a -> b -> a`, and `POST /validation` reports the same thing in its `errors` list.
