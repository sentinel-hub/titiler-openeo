...
| 6 | **Convergence.** `sar_backscatter` consumes injected bands; `calibrate` reduces to arithmetic; delete `_resolve_stack_assets`, `_resolve_polarisation_assets`, `_find_annotation_asset` and the `sar.py:212` rejection (~150 lines). Registers `sar_backscatter` in `reader_requirements._REQUIREMENT_PROVIDERS` — increment 5 left this empty deliberately (its row above). Note found while building increment 5: `COEFFICIENT_LUT` (`sar/annotation.py:157-161`, e.g. `"sigma0-ellipsoid" -> "sigmaNought"`), `BandSource`'s `quantity` strings (`bandsources/sources.py`, e.g. `"sigma_nought"`) and the band-name templates (`"{pol}_sigma0_lut"`) are three different spellings of the same concept with no existing bridge between them — the provider will need to translate `coefficient` through all three, and also needs the polarisations, which live on the *ancestor* `load_collection` node's own `resolved_kwargs["bands"]`, not on the `sar_backscatter` node's own `resolved_kwargs` a provider callable receives (`(resolved_kwargs) -> Requirement`, ADR 0002 §2.6.1) — today `sar_backscatter` instead reads `data.band_names` from its already-materialized input cube (`sar.py:273`) to learn polarisations, which isn't available at plan time. | Abandon if increment 2's gate showed the decomposed read is materially more expensive. Then stop at 3, keep `sar_backscatter` fused, and record why here. |
