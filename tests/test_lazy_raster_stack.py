...
def test_apply_pixel_selection_lazy_optimization():
    """Test that apply_pixel_selection doesn't execute all tasks unnecessarily."""
    ...
    # Create 5 tasks
    assets = [
        {"id": f"item_{i}", "datetime": f"2021-01-0{i+1}T00:00:00Z"} for i in range(5)
    ]

    tasks = [(create_task_with_value(i), assets[i]) for i in range(5)]
    ...
    # Apply pixel selection - this should execute tasks one by one, not all at once
    result = apply_pixel_selection(lazy_stack, pixel_selection="first")
    ...
    # The key optimization: verify that not ALL tasks were executed
    # For 'first' pixel selection, it's likely that early termination occurred
    # so we shouldn't have executed all 5 tasks (depending on the pixel selection algorithm)
    executed_task_count = len(lazy_stack._data_cache)

    # We should have executed at least 1 task but potentially not all 5
    assert executed_task_count >= 1
    assert executed_task_count <= 5

    print(f"Executed {executed_task_count}/5 tasks - optimization working!")
