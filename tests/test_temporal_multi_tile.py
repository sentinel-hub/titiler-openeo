...
def test_multi_tile_temporal_ordering(multi_tile_temporal_stack):
    ...
    for key in keys[:3]:
        assert multi_tile_temporal_stack._timestamp_map[key] == datetime.datetime(2023, 1, 1)
    for key in keys[3:5]:
        assert multi_tile_temporal_stack._timestamp_map[key] == datetime.datetime(2023, 1, 2)
    for key in keys[5:]:
        assert multi_tile_temporal_stack._timestamp_map[key] == datetime.datetime(2023, 1, 3)
