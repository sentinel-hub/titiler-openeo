...
from titiler.openeo.processes.implementations.image import (
    image_indexes, to_array, color_formula, colormap
)
...
from titiler.openeo.processes.implementations.spatial import resample_spatial
...
def test_reduce_dimension_spectral(sample_raster_stack):
    ...
    def mean_reducer(data, **kwargs):
        ...
        elif isinstance(data, dict) and all(isinstance(v, ImageData) for v in data.values()):
