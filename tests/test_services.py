...
def test_service_xyz_access_scopes(app_with_auth, app_no_auth):
    ...
    scope_configs = {
        "private": {"scope": "private", "expected_status": {"owner": 200, "authenticated": 401, "anonymous": 401}},
        "restricted": {"scope": "restricted", "expected_status": {"owner": 200, "authenticated": 200, "anonymous": 401}},
        "public": {"scope": "public", "expected_status": {"owner": 200, "authenticated": 200, "anonymous": 200}},
    }
    ...
    class TestScopesAuth(Auth):
        ...
        def validate_optional(self, authorization: str = Header(default=None)) -> User | None:
            if not authorization:
                return None
            try:
    ...
    for scope_name, config in scope_configs.items():
        app_with_auth.app.dependency_overrides[app_with_auth.app.endpoints.auth.validate] = test_auth.validate
        app_with_auth.app.dependency_overrides[app_with_auth.app.endpoints.auth.validate_optional] = test_auth.validate_optional
        ...
        assert owner_response.status_code == config["expected_status"]["owner"], \
        ...
        assert other_auth_response.status_code == config["expected_status"]["authenticated"], \
        ...
        assert unauth_response.status_code == config["expected_status"]["anonymous"], \
